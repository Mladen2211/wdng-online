    ...
    <div className="relative rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
      <div className="aspect-[3/4] relative overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-6">
          <div
            className={`w-10 h-1 bg-gradient-to-r ${ACCENT_GRADIENTS[accent]} rounded-full mb-3 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500`}
          />
          ...
          <div className="flex flex-wrap gap-2">
            {features.map((feature, i) => (
              <span
                key={i}
                className="px-2 py-1 text-xs text-white/80 bg-white/10 backdrop-blur-sm rounded-full"
              >
                {feature}
              </span>
            ))}
